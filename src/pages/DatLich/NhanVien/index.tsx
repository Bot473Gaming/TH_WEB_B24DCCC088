			...
			render: (record: any) => {
				if (record.ngayLamViec && record.gioLamViec) {
					return (
						<div>
							<div>
								{moment(record.ngayLamViec[0]).format('DD/MM/YYYY')} -{' '}
								{moment(record.ngayLamViec[1]).format('DD/MM/YYYY')}
							</div>
							<div>
								{record.gioLamViec[0]} - {record.gioLamViec[1]}
							</div>
						</div>
					);
				}
					...
					<div>
						...
						<Tooltip title='Xóa'>
							<Popconfirm
								...
								cancelText='Không'
							>
								<Button style={{ marginLeft: 8 }} type='primary' icon={<DeleteOutlined />} />
							</Popconfirm>
						</Tooltip>
					</div>
				);
